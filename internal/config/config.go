...
type Config struct {
	...
	// tier exactly as today. When true, the server wraps the shared backend in
	// a redis.TierDispatchBackend that routes non-Team tiers to a shared ACL
	// carve and the Team tier to a dedicated pod — the durable fix for the
	// dedicated-pod-per-token capacity problem. Off until an operator opts in.
	RedisTierAwareRoutingEnabled bool   // REDIS_TIER_AWARE_ROUTING_ENABLED, default false
...
func Load() *Config {
	cfg := &Config{
		...
		// K8sDedicatedBackend.
		RedisTierAwareRoutingEnabled: os.Getenv("REDIS_TIER_AWARE_ROUTING_ENABLED") == "true",
